...
function Share({
    ...
    const mapState = window.MapStoreAPI?.getMapState() ?? {zoom:0,center:{x:0,y:0}};
    const latTrim = Math.round(mapState.center.x * 100000) / 100000
    const lngTrim = Math.round(mapState.center.y * 100000) / 100000
        ...
        >
            ...
            >
                ...
                <div className="gn-share-panel-body">
                    ...
                    <SharePageLink url={`${pageUrl}?zoom=${mapState.zoom}&lat=${latTrim}&lng=${lngTrim}`} label="url link with lat/lon and zoom"/>
                    <SharePageLink url={embedUrl} label={<Message msgId={`gnviewer.embed${resourceType}`} />} />
                    <SharePageLink url={`${embedUrl}?zoom=${mapState.zoom}&lat=${latTrim}&lng=${lngTrim}`} label={<Message msgId={`gnviewer.embed${resourceType}`}/>} />
